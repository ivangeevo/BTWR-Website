...
import { COOK_FOOD_COST, EAT_XP_REWARD } from "./resources";
...
export default function Campfire() {
  ...
  useEffect(() => {
    if (!mounted) return;
    setStage(currentCampfireStage(campfire));
    const id = window.setInterval(() => setStage(currentCampfireStage(campfire)), REFRESH_MS);
  ...
  function handleCook() {
    if (completeCooking()) flash(`${resourceMeta.cookedFood.icon} +1 ${resourceMeta.cookedFood.name}`);
  }

  function handleEat() {
    if (eatCookedFood()) flash(`+${EAT_XP_REWARD} XP`);
  }

  // Cooking only exists once Food is actually gatherable — the same
  // admin-configured tier that reveals Gathering's Hunting toggle
  // (features.huntingTierId), so the two stay in step from one setting.
  const foodTier = tiers.find((t) => t.id === features.huntingTierId) ?? tiers[tiers.length - 1];
  const foodAvailable = isTierUnlocked(foodTier.id);

  const canCook = stage === 3 && resources.food >= COOK_FOOD_COST && remainingMs <= 0;
  ...
  return (
    <div className="outpost-panel rounded-xl p-5">
      ...
      {stage === null ? (
        ...
        <>
          <div className="mt-3 flex items-center gap-3">
            <span className="campfire-icon text-3xl leading-none" data-stage={stage} aria-hidden="true">
              {CAMPFIRE_ICONS[stage]}
            </span>
            <p className="font-semibold text-white">{CAMPFIRE_STAGES[stage]}</p>
          </div>
          <p className="mt-3 text-sm text-slate-300">{CAMPFIRE_CAPTIONS[stage]}</p>
          <button
            type="button"
            onClick={() => {
              tendCampfire();
            ...
            className="btn-glow btn-gradient mt-4 rounded-lg px-4 py-2 text-sm font-semibold text-white"
          >
            Tend the Fire
          </button>
            ...
            <div className="mt-4 border-t border-white/10 pt-4">
              ...
              <div className="mt-2 flex gap-2">
                <button
                  ...
                  onClick={handleCook}
                  disabled={!canCook}
                  className={`flex-1 rounded-md border px-2.5 py-2 text-xs font-semibold transition-colors ${
                    canCook
                      ? "border-[var(--outpost-accent)] text-[var(--outpost-accent)] hover:bg-[var(--outpost-accent-soft)]"
                      : "border-white/15 text-white/30"
                  }`}
                >
                  Cook (-{COOK_FOOD_COST} Food)
                </button>
                <button
                  type="button"
                  onClick={handleEat}
              ...
              {stage === 3 && resources.food < COOK_FOOD_COST && (
                <p className="mt-2 text-[11px] text-white/35">Needs at least {COOK_FOOD_COST} Food — go hunting.</p>
              )}
              {feedback && <p className="mt-2 text-xs text-[var(--outpost-accent)]">{feedback}</p>}
